...
# Controlled ablation: current LoRA-adapted CLS geometry, prototype gate only.
# Runs sequentially on one GPU: 3 datasets x seeds 1/42/88 = 9 runs.
...
WARMUP_EPOCHS="5"
UPDATE_INTERVAL="5"
SUMMARY_DIR="outputs/analysis/dynamic_prototype_only_asym40_s42_validation"
...
preflight() {
  ...
  for item in "${DATASETS[@]}"; do
    ...
    [[ ! -e "$input_dir/checkpoint_validation_dynamic_proto_only_p04_s${VALIDATION_SEED}" ]] || die "$name output already exists; refusing overwrite."
  done
}

preflight
echo "[Dynamic Prototype Gate Only] fixed_clean_validation_v1; p=$FIXED_P; warmup=$WARMUP_EPOCHS; interval=$UPDATE_INTERVAL"
echo "[Dynamic Prototype Gate Only] sequential plan: 3 datasets x seeds $SEEDS = 9 runs"

for item in "${DATASETS[@]}"; do
  IFS='|' read -r name input_dir noise_index <<< "$item"
  output_dir="$input_dir/checkpoint_validation_dynamic_proto_only_p04_s${VALIDATION_SEED}"
  mkdir -p "$output_dir"
...
"$PYTHON_BIN" tools/summarize_dynamic_prototype_multiseed.py \
  --method-key dynamic_proto_only \
  --dataset "CUB-200-2011=outputs/CUB_200_2011/CUB-200-2011-asym40-s42/v1_cub_asym_r0p4_s42/checkpoint_validation_dynamic_proto_only_p04_s${VALIDATION_SEED}" \
  --dataset "Stanford Cars=outputs/Stanford_Cars/Stanford-Cars-asym40-s42/v1_cars_asym_r0p4_s42/checkpoint_validation_dynamic_proto_only_p04_s${VALIDATION_SEED}" \
  --dataset "FGVC-Aircraft=outputs/FGVC_Aircraft/FGVC-Aircraft-asym40-s42/v1_aircraft_asym_r0p4_s42/checkpoint_validation_dynamic_proto_only_p04_s${VALIDATION_SEED}" \
  --seeds "$SEEDS" \
  --output-dir "$SUMMARY_DIR"

echo "[DONE] 9 Dynamic Prototype Gate Only runs completed; summary: $SUMMARY_DIR"
